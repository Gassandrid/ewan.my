---
tags:
  - engineering/3dprinting
  - projects/homelab/hardware
date: 2025-08-31
updated: 2025-10-21
---

My current primary **homelab** as per se - it is feature complete in terms of hardware now, the only thing I could really see to add now would be a zig-bee dongle and and SDR for passive RF monitoring. Most work now consists of [[Saturn Rack Software]].

---

## Finished Product

---

## Hardware

For hardware I can stick with the **Mini PC** i have for now, but eventually want to invest in a full solution like the video below:

https://www.youtube.com/watch?v=wbRViRwflbI

I want to implement 3d min pc nodes with the structure from [[This homelab setup is my favorite one yet.]]

I am thinking of two [[Venus UM790 Pro Mini PC]]'s and a mini nas like the [[Beelink ME Mini NAS]] which is perfect for a 10 inch rack. However, I do not yet have a need for any more compute, so the **priority would be setting up the mini nas beelink.**

---

## Software

Software is documented at [[Saturn Rack Software]], but the main things would be some basic NAS/cloud storage tech, but also room for experiments etc.

---

## Materials and Design - Done

As the [makerworld page](https://makerworld.com/en/models/1381701-saturn-v-u-diy-10-network-rack#profileId-1430257) outlines, there are a few things we will need:

### Makers Supply

- [x] [M3x5x4 Round Threaded Brass Heat-Insert Nut (20PCS) - AB012](https://store.bambulab.com/products/round-threaded-brass-heat-insert-nut?id=42410687529096&modelId=1381701) × 2
- [x] [M3x10 BHCS Machine Screw (20PCS) - AA172](https://store.bambulab.com/products/m3-button-head-cap-machine-screws-bhcs?id=42338644820104&modelId=1381701) × 1
- [x] [M3x20 BHCS Machine Screw (10PCS) - AA061](https://store.bambulab.com/products/m3-button-head-cap-machine-screws-bhcs?id=42338644983944&modelId=1381701) × 1
- [x] [M3x8 BHCS Machine Screw (20PCS) - AA058](https://store.bambulab.com/products/m3-button-head-cap-machine-screws-bhcs?id=42338644787336&modelId=1381701) × 2
- [x] [M3 Carbon Steel Hex Nut (20PCS) - AB005](https://store.bambulab.com/products/hex-nut?id=42410684022920&modelId=1381701)

### External

- [x] [10U Steel Rack Rails](https://www.amazon.com/dp/B08RYQJZZ8?smid=AQNFMF4UZ1II6&th=1) x 4

### Filament

- [x] black pla (already have)
- [x] white pla

### Pieces to Print

- [x] 1 - Front rails x 2
- [x] 2 - Front rails x 2
- [x] 3 - Side rail x 1
- [x] 4 - Side rail x 1
- [x] 5 - Side rail x 1
- [x] 6 - Side rail x 1
- [x] 7 - Side mounting x 1
- [x] 8 - Side mounting x 1
- [x] 9 - Side mounting x 1
- [x] 10 - Side mounting x 1
- [x] 11 - Side panel 2u wavy x 1
- [x] 12 - Side panel 2u wavy x 1
- [x] 13 - Side panel 2u door x 1
- [x] 14 - Side panel 2u door x 1
- [x] 15 - Side panel 4u x 1 
- [x] 16 - Side panel 4u x 1 
- [x] 17 - Mounting clip x 24
- [x] 18 - Handle x 2

### Assembly Manual

![[Assembly Manual.pdf]]

### Extra Parts

I like some of the ideas for mountings from [this video](https://www.youtube.com/watch?v=t5ZMkzbdSR8), namely the ones for the mini pcs, will look into them.
