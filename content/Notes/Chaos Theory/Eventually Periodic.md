...
updated: 2026-02-17T11:21:16-05:00
---

An orbit $\{ x_1, x_2, \dots \}$ is **eventually periodic** if it lands *exactly* on a periodic orbit after finitely many steps and cycles from there. Formally, $\exists$ integers $N \geq 0$, $k \geq 1$ such that:
...
The canonical example: under $f(x) = 3x \pmod{1}$, all rationals are EP. The map acts as a left-shift on base-3 digits, so any eventually-repeating base-3 expansion hits a cycle in finite steps (proven in HW2). e.g.
