...
class:
  - note
---
Recall that $\nabla f$ is [[Orthogonality|orthogonal]] to the level curves/surfaces.
...
This **key formula** tells us the the **gradient vector**(at any point) is [[Orthogonality|orthogonal]] to the tangent vector (at the same point) to any curve on the surface passing through that point.
