...
1. Because the point is on a level curve of g, the gradient of g is [[Orthogonality|orthogonal]] to the direction of the curve:
...
2. Because the point is a local max or min of f along the curve, the gradient of $f$ is also [[Orthogonality|orthogonal]] to the direction of the curve:

$$
\nabla f(x_0, y_0) \cdot \vec{r}'(t_0) = 0
$$

Both gradients, $\nabla f$ and $\nabla g$, are [[Orthogonality|orthogonal]] to the very same tangent vector $r'(t_{0})$ at that point. In $\mathbb{R}^2$, this means that the two gradient vectors must be **parallel**.
