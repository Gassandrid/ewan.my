---
date: 2025-09-08
updated: 2025-09-08
class:
  - note
  - lectureNote
tags:
  - university
  - biology/cells
course: "[[Principles of Biology 1]]"
lesson:
  - lecture
  - "`"
lecture-number: 6
source:
related:
author:
---

**Eukaryotic Cells** are a type of [[Cell]] that have a membrane-bound nucleus and other membrane-bound organelles. They are typically larger and more complex in structure than prokaryotic cells. Eukaryotic cells are found in all multicellular organisms, including plants, animals, fungi, and protists.

![[Pasted image 20250908210538.png]]

Unlike [[Prokaryotic Cells]], eukaryotic cells have a true nucleus that is enclosed by a double membrane called the nuclear envelope. The nucleus contains the cell's genetic material (DNA) and is the site of transcription (the process of making RNA from DNA).
